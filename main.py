...
def bill_button_operation():
    ...
    if ans=="yes":
        ...
        week_day_dict = {0:"Segunda",1:"Terça",2:"Quarta",3:"Quinta",4:"Sexta",5:"Sábado",
                            6:"Domingo"}
        st += f"{t.tm_mday} / {t.tm_mon} / {t.tm_year} ({week_day_dict[t.tm_wday]})"
        st += " "*10 + f"\t\t\t\t\t\tHoras: {t.tm_hour} : {t.tm_min} : {t.tm_sec}"

        #Customer Name & Contact
        st += f"\nNome Completo: {customer_name}\nTelefone: {customer_contact}\n"
        st += "-"*130 + "\n" + " "*4 + "Produto\t\t\t\t\tTaxa\tQuantidade\t\tValor\n"
        st += "-"*130 + "\n"
...
title_frame = Frame(root, bd=8, bg="#074463", relief=GROOVE)
...
#===============Menu===============
menu_frame = Frame(root,bd=8, bg="#074463", relief=GROOVE)
menu_frame.place(x=0,y=125,height=585,width=680)

menu_category_frame = Frame(menu_frame,bg="#074463",pady=10)
...
show_button.grid(row=0,column=2,padx=94)

show_all_button = ttk.Button(menu_category_frame, text="Tudo",
...
menu_tabel.heading("name",text="Nome")
menu_tabel.heading("price",text="Preço(R$)")
...
#===============Item Frame=============
item_frame = Frame(root,bd=8, bg="#074463", relief=GROOVE)
...
add_button.grid(row=0,column=0,padx=28,pady=20)

remove_button = ttk.Button(item_frame3, text="Remove Item"
                        ,command=remove_button_operation)
remove_button.grid(row=0,column=1,padx=40,pady=20)

update_button = ttk.Button(item_frame3, text="Atualiza Quantidade"
                        ,command=update_button_operation)
update_button.grid(row=0,column=2,padx=40,pady=20)

clear_button = ttk.Button(item_frame3, text="Limpar",
                        width=8,command=clear_button_operation)
clear_button.grid(row=0,column=3,padx=40,pady=20)

#==============Order Frame=====================
order_frame = Frame(root,bd=8, bg="#074463", relief=GROOVE)
...
order_tabel.heading("name",text="Nome")
order_tabel.heading("rate",text="Taxa (R$)")
order_tabel.heading("quantity",text="Quantidade")
order_tabel.heading("price",text="Preço(R$)")
...
bill_button = ttk.Button(order_frame, text="Nota",width=8,
                        command=bill_button_operation)
bill_button.pack(side=LEFT,anchor=SW,padx=80,pady=10)

cancel_button = ttk.Button(order_frame, text="Cancelar",command=cancel_button_operation)
